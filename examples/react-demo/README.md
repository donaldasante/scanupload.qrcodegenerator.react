...
A minimal Vite + React app that integrates
...
`build:react` builds the core dependency and the React package, including the
stylesheet imported by this demo. Re-run it after changing either package.

The app starts on https://localhost:5173 (HTTPS is required for the hub's
`Origin` checks).
...
- `src/override.css` — package CSS overrides (themed background, button colour,
  etc.)
