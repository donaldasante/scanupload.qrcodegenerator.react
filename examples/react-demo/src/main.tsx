...
function App() {
  ...
  useEffect(() => {
    ...
    instance.use(Dashboard, {
      ...
      // style, so the host is what index.css controls — and the host mirrors
      // the QR square's measured height.
      height: "100%",
    });
    ...
    // StrictMode mounts effects twice in development; destroying here keeps the
    // second mount clean rather than leaving two sessions running.
    return () => {
      instance.destroy();
      setCore(null);
      setUppyFileCount(0);
  ...
  const measureScan = useCallback((): void => {
    ...
    // Re-point the observer rather than leaving it watching a detached node, in
    // case the widget ever swaps the square for a new element.
  ...
  return (
    <>
        ...
        <div className="mb-6">
          <div className="flex flex-col">
            ...
            <div className="flex flex-col gap-0 mt-2">
              ...
              <div className="flex flex-row items-start mt-2 space-x-4">
                {/* items-start (not items-center) so the radio stays aligned at
                    the top of the label, even when the label text wraps to two
                    lines like "X-Large". */}
      ...
      {/*
        One card, two panels: the QR code for uploads from a phone on the left,
        the Uppy Dashboard for files from this device on the right.
      */}
