...
```text
The origin 'https://localhost:5176' is not in the AllowedOrigins list for tenant '...'.
```
...
The browser creates the ScanUpload session with an HTTPS request, then uses SignalR over a secure WebSocket. Most production connection failures are caused by browser security policy or an origin mismatch rather than the Svelte integration.
