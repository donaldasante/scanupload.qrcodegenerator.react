 ...
 * sends and answers with the JSON shape `getResponseData` in `app.component.ts`
 * expects.
 *
    ...
    return {
        plugins: [
            angular({
                ...
                /**
                 * The demo loads the runtime compiler (`import '@angular/compiler'`
                 * in `main.ts`) because it has no ahead-of-time setup, so the
                 * plugin has to be told that. Left at its default, the plugin
                 * assumes AOT: its build optimizer inlines `ngJitMode: false`
                 * and marks every Angular bundle — `@angular/compiler`
                 * included — side-effect free, so the compiler is dropped from
                 * the production build and the app dies at bootstrap with
        ...
        server: {
            // One port per demo: 5173 is react-demo, 5174 vanilla-js,
