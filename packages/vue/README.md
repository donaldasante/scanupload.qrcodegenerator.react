...
Vue 3 component wrapper for the ScanUpload QR Code Generator. Renders a QR code,
...
The `client-id` prop identifies your tenant. Create it in the ScanUpload
Dashboard:
...
import {
  DownloadButton,
  useQrCodeCore,
} from "@scanupload/qr-code-generator-vue";

const { core } = useQrCodeCore({ sessionUrl: "/api/front-end/session" });
</script>
...
Set `:show-file-previews="false"` and use the lifecycle callbacks when something
...
<QrCodeGenerator
  ...
  :on-file-available="(file, origin) => add(file)"
  :on-file-removed="(file) => drop(file.id)"
  :on-files-cleared="() => dropAll()"
/>
```
...
- `useQrCodeCore` — composable returning
  `{ state, core, retrySession, setOptions }`
