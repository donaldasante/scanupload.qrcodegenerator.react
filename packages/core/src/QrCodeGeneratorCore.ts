...
export type QrCodeGeneratorCoreSetOptions = Partial<
  Pick<QrCodeGeneratorCoreOptions, "sessionUrl" | "refreshTokenUrl">
>;
...
export class QrCodeGeneratorCore {
  ...
  private readonly _debouncedGetData = debounceAsync(
    () => this._getData(),
    1000,
  );
  ...
  constructor(options: QrCodeGeneratorCoreOptions) {
    ...
    this._lastSessionIds =
      this._storage.getItem<string[]>(LAST_SESSION_IDS_KEY) ?? [];
  }
  ...
  private async _getSessionInformationAsync(
    signal?: AbortSignal,
  ): Promise<void> {
  ...
  private async _createHubConnectionAsync(
    hubUrl: string,
    ...
    try {
      ...
      connection.on("FileProgress", (fileId: string, progress: number) => {
        this._setState({
          uploadedFiles: this._state.uploadedFiles.map((file) =>
      ...
      connection.onreconnected(async (connectionId) => {
        ...
        try {
          ...
          const prevMap = new Map(
            this._state.uploadedFiles.map((f) => [f.id, f]),
          );
