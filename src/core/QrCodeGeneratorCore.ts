...
import { getItem, setItem } from '../react/hooks/localStorage';
...
export class QrCodeGeneratorCore {
    ...
    constructor(
        ...
        this._lastSessionIds = getItem(LAST_SESSION_IDS_KEY) ?? [];
    }
    ...
    private async _getSessionInformationAsync(signal?: AbortSignal): Promise<void> {
        ...
        try {
            ...
            setItem(LAST_SESSION_IDS_KEY, this._lastSessionIds);
        } catch (error) {
            if (signal?.aborted) return;
